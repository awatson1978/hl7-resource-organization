    ...
    if (req.body) {
      ...
      if(organization){
        ...
        organizationId = Organizations.update({_id: req.params.id}, {$set: organizationUpdate },  function(error, result){
          if (error) {
            //console.log('PUT /fhir/Organization/' + req.params.id + "[error]", error);
...
JsonRoutes.add("post", "/fhir-1.6.0/Organization", function (req, res, next) {
  ...
  if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
    ...
    if (req.body) {
      ...
      if (newOrganization.birthDate) {
        newOrganization.birthDate = moment(newOrganization.birthDate);
      }

      newOrganization = Organizations.toMongo(newOrganization);

      process.env.DEBUG && console.log('newOrganization', JSON.stringify(newOrganization, null, 2));
      // process.env.DEBUG && console.log('newOrganization', newOrganization);
...
JsonRoutes.add("get", "/fhir-1.6.0/Organization", function (req, res, next) {
  ...
  // console.log('GET /fhir/Organization', req.query);
  ...
  if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
    ...
    // var searchLimit = 1;
    // var organizationData = Organizations.fetchBundle(databaseQuery);
